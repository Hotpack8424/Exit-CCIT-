...
def check_url(url, depth=0, max_depth=10, results={'accessible': 0, 'blocked': 0}, retry_delay=200):
    ...
    try:
        ...
        elif response.status_code == 403:  # 403 상태 코드를 받은 경우, 일정 시간 대기 후 재시도
            print(f"{url} 잠시 대기 후 재시도합니다...")
            time.sleep(retry_delay)  # 설정된 대기 시간(초)만큼 대기
            response = requests.get(url, allow_redirects=True)  # 재시도
            if response.status_code >= 200 and response.status_code < 300:
                print(f"{url} 접속 가능")
                results['accessible'] += 1
            else:
